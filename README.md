...
## Getting Started
The gh2jira utility requires a `config.yaml` file with Github Token, Jira Personal Access Token, Jira URL and Jira Username.Example `config.yaml` file:

```yaml
schema: gh2jira.config
jiraBaseUrl: blah
jiraUsername: blah@blah.com
authTokens:
  jira: foo
  github: baz
```
...
Specify the required Jira URL in the `config.yaml` file under the key `jiraBaseURL` and jira username unser the key `jiraUsername`
### Build the Utility
